...
impl Default for SimServerController {
  fn default() -> Self {
    Self::new()
  }
}
...
impl ServerController for SimServerController {
  ...
  async fn shutdown_server(&self) -> Result<(), Box<dyn ThreadSafeError>> {
    ...
    // systemctl stop waits until shutdown completes, so simulate the waiting
    // period by waiting to respond.
    time::sleep(OP_DELAY).await;
    Ok(())
  }
}
